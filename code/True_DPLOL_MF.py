...
# 处理列和小于1的函数
    ...
    def tripletlap(self,D, P, k):
        ...
        dist1 = self.EuDist2(D.T)  # 综合字典原子间的距离矩阵，r*r

        dist2 = self.EuDist2(P)  # 解析字典原子间的距离矩阵，r*r
        ...
        G = G1 + G2 #局部序信息引入了综合字典和解析字典
        # G=G2 #局部序信息仅仅引入到解析字典中
        G[G > 1] = 1
        # print(G)
        C = C1 + C2
        # C=C2
    ...
    def initilization(self, MSData, id, H, dict_size):
        ...
        for h in range(H):
            ...
            for c in range(self.nclass):
                ...
                classA[h][c] = temData #取第c类样本 m*n
                classD[h][c] = self.normcol_equal(np.random.randn(dim, dict_size))  # m*r
                classP[h][c] = self.normcol_equal(np.random.randn(dim, dict_size)).T #r*m
    ...
    def MSDPL(self,MSData, id, H, class_count):
        # 初始化综合字典 D 和分析字典 P 同时利用更新公式得到系数矩阵 X
    ...
    def classification_ADL(self,test_data, D, P, samp_freq):
        ...
        code_z = self.code_num(test_data, f)#寻找视图的对应信息
        ...
        for i in range(sample_num):
            ...
            #     print("error:",ert_ci[c])
            # print("===================================")
            # txet="样本"+str(i)
            # print(txet,min(ert_ci, key=ert_ci.get))
    ...
    def print_X(self, X):
        ...
        for i in range(self.nclass):
            ...
            tempx=pca.fit_transform(Data[i])
            x.append(tempx)
            tempindex=[]
            for idx in np.random.choice(range(np.array(Data[i]).shape[0]), size=20, replace=False, p=None):
                tempindex.append(idx)
            selectIdx.append(tempindex)
            E.append(x[i][selectIdx[i]])

            plt.scatter(E[i][:, 0], E[i][:, 1], c=color[i])
        plt.show()
    ...
    def run(self):
        ...
        for h in range(self.H):
            ...
            h_dimension[h] = np.shape(MSData[h])[1]# 训练集维度
            label[h] = np.array(pd.read_csv(self.labelfile[h], sep="\t", header=None)).astype(np.int)[0, :]
            true_testlabel[h] = np.array(pd.read_csv(self.true_testlabel_file[h], sep="\t", header=None)).astype(np.int)[0, :]
        ...
        for i in range(10):
            # 字典学习阶段
            tr_start = time.clock()
            D, P, X = self.MSDPL(MSData, id, self.H, class_count)
...
if __name__ == '__main__':
    ...
    dicnum = 18
    ratio={0:1,1:21,2:63}
    model = True_DPLOL_MF(iternum, H, nclass, dicnum, trainX, trainY, testX, testY,ratio)
    acc,f1=model.run()
