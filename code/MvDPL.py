...
# 按列拼接
    ...
    def initilization(self,MSData, id, H, dict_size):
        ...
        for h in range(H):
            ...
            contact_D[h] = reduce(self.contacth, [classD[h][c] for c in range(self.nclass)])
    ...
    def MSD_SADL(self,MSData, id, H):
        ...
        # pref_error = all_energy(MSData,D,datamat,contact_D,P,X,id)
        ms_iter = 1
    ...
    def run(self):
        ...
        for i in range(10):
            # 训练学习
            tr_start = time.clock()
            D, P = self.MSD_SADL(MSData, id, self.H)
            tr_time = (time.clock() - tr_start)
            # print(D, P)

            # 测试样本打标
            tt_start = time.clock()
            test_label = self.classification_ADL(z, D, P, samp_freq)
            tt_time = (time.clock() - tt_start)
...
if __name__ == '__main__':
    ...
    dicnum = 18
    ratio = {0: 1, 1: 1, 2: 1}
    model = MvDPL(iternum, H, nclass, dicnum, trainX, trainY, testX, testY, ratio)
    acc, f1 = model.run()